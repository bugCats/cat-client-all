...
public class CatMethodInterceptor implements MethodInterceptor {
     ...
     * 1、处理入参
     *   
     *   入参格式为：post、get键值对（form表单提交方式），或者使用post发送json字符串
     *   
     *   因此需要将入参进行转换：
     *      
     *      如果方法上有多个有效入参，
     *          
     *          只能假设这些入参，全部是基础数据类型；（如果这些有效参数，存在对象，都会有bug。碰到这种情况，建议使用post发送json字符串，或者将对象转成json字符串，再使用）
     *      
     *          创建一个map，key=参数名称，value=参数值；（这就是基础数据类为什么要使用@RequestParam注解原因，interface编译成class之后，不保留参数名称）
     * 
     *          返回这个map，最后转成key=value&key2=value2键值对，或者json字符串
     *          
     *      如果只有一个有效参数，
     *          
     ...
     *              
     *              最后将对象转换成key=value&key2=value2键值对，或者json字符串
     *    
     *    
     *    
     * 2、发送前预处理
     ...
     *      
     *      转换入参，根据请求方式，将入参转换成最终形式：键值对 -> Map ；postJson -> json字符串
     * 
     * 
     * 
     * 3、发送http请求
     ...
     *              是复杂对象，通过fastjson转换
     *              
     *              开启了包裹类模式，并且返回参数不等于包裹类，需要设置TypeReference
     *              
     *              
     *              
     * 6、最后处理
     * 
     *      如果开启了包裹类模式，并且返回参数不等于包裹类，进行拆分、分离业务对象
     *    
     * 
     * 
     * 7、返回最终结果
    ...
    public Object intercept (Object target, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
        ...
        param.setCatHttp(factory.getCatHttp());
